...
def login_process():
    ...
    # First check if user is local. Local account take priority.
    user = UserModel.query.filter(and_(func.lower(UserModel.username) == func.lower(username), UserModel.ldap == 0)).first()
    if user:
        if not account.validate_password(user.password, password):
        ...
        user = account.get_ldap_user(ldap_user['username'])
        if not user:
            # Create
            user = account.create_ldap_user(ldap_user['username'], ldap_user['fullname'], ldap_user['email'])
            if not user:
    ...
    user = account.login_session(user)
    login_user(user)
    account.record_login(user.id)
...
def logout():
    ...
    account.logout_session(current_user.id)
    logout_user()
...
def ldap_changepwd():
    ...
    user = account.get_ldap_user(username)
    if not user:
        session.pop('ldap_username', None)
        session.pop('ldap_time', None)
        return redirect(url_for('auth.login', next=next))
